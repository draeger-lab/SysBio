...
/**
 * <p>
 * When reading in files and processing each line (files line by line), this
 * class is handy to output the percentage of how much is done.
 * </p>
 * 
 * <p>
 * NOTE: Theoretically, this class can be used for displaying more types of
 * progress than just reading in files.
 * </p>
 * 
...
public class FileReadProgress {
   ...
   */
	 ...
	 * @param file
	 *            the file to be read from
	 * @param printProgressInSameLine
	 *            if <code>true</code>, the output will always be in the same
	 *            line
	 */
	 ...
	 * the given file. Additionally, it has to be specified if the progress
	 * should be written in the same line (default) or a new line for each
	 * percentage.
	 * 
	 * @param filepath
	 *            the path of the file
	 * @param printProgessInSameLine
	 *            if <code>true</code>, the output will always be in the same
	 *            line
	 */
	...
	/**
	 * Returns if the current percentage should be put out every time it is
	 * updated.
	 * 
   ...
   */
	...
	public void progress(long bytesRead) {
		...
		if (outputPercentage) {
			...
			if (printProgressInSameLine) {
				try {
					System.out.write(getDisplayBarString(progressCounter, perc)
							.getBytes());
	 ...
	 * adds 1 to the length of the given string to account for newline
	 * characters.
	 * 
	 * @param curLine
	 */
	...
	/*
	 * Kommentiert. Methode macht kein Sinn! Man kann immer (ob console oder
	 * nicht) in selbe Zeile Printen! protected boolean canPrintInSameLine() {
	 * return printProgressInSameLine && (System.console() != null) &&
	 ...
	 * updated. <code>true</code> means, the percentage will be put out every
	 * time it is updated, <code>false</code> means, no output will be
	 * automatically generated, but {@link DiplayBar} has to be called
	 * explicitly.
	 * 
	...
	/**
	 * Determins if ANSI compliance console commands can be used, based on java
	...
	protected boolean useSimpleStyle() {
		...
		if (isWindows)
			useSimpleStyle = true; // MS Windows has (by default) no ANSI
									// capabilities.
