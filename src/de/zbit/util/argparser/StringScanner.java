...
class StringScanner {
	...
	public String scanQuotedString() throws StringScanException {
		...
		if (idx == len) {
			...
			if (exception == null && idx >= len) {
				...
				exception = new StringScanException(idx, "unclosed quoted string");
			} else {
				idx++;
			}
		} else {
	...
	public String scanNonWhiteSpaceString() throws StringScanException {
		...
		skipWhiteSpace();
		if (idx == len) {
			StringScanException e = new StringScanException(idx, "end of input");
			idx = idxSave;
			throw e;
		} else {
			c = buf[idx++];
			...
			if (Character.isWhitespace(c) || stringDelimiters.indexOf(c) != -1) {
				idx--;
			} else {
				sbuf.append(c);
			}
	...
	public long scanInt(int radix, boolean skipWhite) throws StringScanException {
		...
		if (idx >= len) {
			...
			exception = new StringScanException(idx - 1, "malformed "
			...
			if (Character.isLetter(c = buf[idx]) || Character.isDigit(c) || c == '_') {
				exception = new StringScanException(idx, "malformed " + baseDesc(radix)
						+ " integer");
			}
		}
		if (exception != null) {
...
//						signed = true;
		...
		if (exception == null) {
			...
			{
				try {
					value = Double.parseDouble(new String(buf, idxSave, idx - idxSave));
				} catch (NumberFormatException e) {
					exception = new StringScanException(idx, "malformed floating number");
				}
			}
		}
		if (exception != null) {
	...
	public boolean scanBoolean() throws StringScanException {
		...
		if (exception == null) {
			if (i < testStr.length() || Character.isLetterOrDigit(c = buf[idx])
					|| c == '_') {
