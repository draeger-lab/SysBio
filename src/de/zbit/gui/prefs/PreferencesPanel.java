...
public abstract class PreferencesPanel extends JPanel implements KeyListener,
  ...
  List<Option<?>> addOptions(LayoutHelper lh, Iterable<? extends Option> options, Map<Option<?>, OptionGroup<?>> deleteFromHere) {
		...
		for (Option<?> option : options) {
			...
			JComponent jc = properties.containsKey(option) ?
			    getJComponentForOption(option, properties, this) : null;
			if (jc != null) {
  ...
  private static Option<?> getOptionForJComponent(JComponent toCompare, Component currentRecurse) {
    
    /* This does NOT work for JLabeledComponents and FileSelectors.
     * FIXED: => Avoided by adding the getOptionForAutoGeneratedJComponent() method.
     * 
     ...
     * 
     */
    if (currentRecurse instanceof JComponentForOption || JComponentForOption.class.isAssignableFrom(currentRecurse.getClass())) {
      return ((JComponentForOption) currentRecurse).getOption();
    } else if (currentRecurse instanceof PreferencesPanel || PreferencesPanel.class.isAssignableFrom(currentRecurse.getClass())) {
      Option<?> ret = ((PreferencesPanel)currentRecurse).getOptionForAutoGeneratedJComponent(toCompare);
      if (ret!=null) return ret;
    } if (currentRecurse.getParent()!=null && (!currentRecurse.getParent().equals(currentRecurse))) {
