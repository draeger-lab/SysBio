...
public class MultipleTableView<T extends TableModel> extends JPanel implements Iterable<T> {
	...
	public void removeTable(String title) {
		...
		if (index < 0) {
			throw new IndexOutOfBoundsException(String.format(ResourceManager.getBundle(StringUtil.RESOURCE_LOCATION_FOR_WARNINGS)
				.getString("NO_TAB_WITH_TITLE"), title));
		} else {
		  removeTable(index);
		}
	}
