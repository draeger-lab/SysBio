 ...
 * java.awt.image, java.awt.image.BufferedImage, etc.
 * 
 * @author wrzodek
 */
public class ImageTools {
   ...
   * @param bimg - image to resize
   * @param w - desired with
   * @param h - desired height
   * @param scaleImageProportional - if set to true, width and height will be set to the closest
   * of the given value, such that either with=w or height=h and the image proportions are being
   * preserved.
   * @return resized BufferedImage
   */
  public static BufferedImage resizeImage(BufferedImage bimg, int w, int h, boolean scaleImageProportional) { // -1 = proportional
    if (!scaleImageProportional) {
      Image img = bimg.getScaledInstance(w, h, Image.SCALE_SMOOTH); // width, height, hints(Fast/Smooth)
      return image2BufferedImage(img,false);
    } else {
      // resize proportional (ausgehend von max), und zentriert in buffered image mit gegebenen koords setzen).    
      int diffX = Math.abs(bimg.getWidth()-w);
      int diffY = Math.abs(bimg.getHeight()-h);
      
      int h2=h,w2=w;
      if (diffX>diffY) h2=-1; else w2=-1;
      Image img = bimg.getScaledInstance(w2, h2, Image.SCALE_SMOOTH); // width, height, hints
   ...
   * @return BufferedImage
   */
  public static BufferedImage image2BufferedImage(Image img, boolean drawBorder) {
    BufferedImage bmage =new BufferedImage(img.getWidth(null),img.getHeight(null),BufferedImage.TYPE_INT_ARGB);
  ...
  /**
   * Centers the image within the given coordinates (does NOT scratch or other things).
   * @param img - image to paing
   * @param targetWith
   * @param targetHeight
   ...
   * @return BufferedImage
   */
  public static BufferedImage image2BufferedImage(Image img, int targetWith, int targetHeight, boolean autoSetBackgroundColor, boolean drawBorder) {
    BufferedImage bmage =new BufferedImage(targetWith,targetHeight,BufferedImage.TYPE_INT_ARGB);
    Graphics g = bmage.getGraphics();
