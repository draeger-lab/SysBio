...
public class KeggAdaptor implements Serializable {
	...
	public static void main(String[] args) {
		...
		System.out.println("======================");
		String genesString = adap
				.getIdentifier("hsa:9693 hsa:994 hsa:9965 hsa:998");
		...
		String weat = adap
				.get("cpd:C00103 hsa:8491 rn:R05964 q:243 glycan:G00181");
	 ...
	 * @param pathway_id
	 *            e.g. 'path:hsa00650'
	 * @return String array with all genes of the pathway, e.g. 'hsa:10327',
	 *         'hsa:123'
	 */
	...
	public String[] getGenesByPathwayWithTimeout(String pathway_id)
	...
	private void retrieveVariousIDs(String in_id) { // z.B. "hsa:8491"
		...
		for (String s : split) { // NCBI-GeneID: 8491
			...
			for (int k = 0; k < key.length; k++) {
				...
				String te = r
						.substring(pos + ke.length(), r.indexOf("\n", pos))
						.trim();
				if (te.length() != 0)
		...
		for (int k = 0; k < key.length; k++)
			if (last_id_results[k].length() > 0
					&& last_id_results[k].endsWith(","))
				last_id_results[k] = last_id_results[k].substring(0,
						last_id_results[k].length() - 1);
	}
	...
	public String get(String id) {
		...
		// Wenn mehrere Threads/ Instanzen gleichzeitig laufen, kommts zu
		// starken
	 ...
	 * 
	 * @param a
	 *            string with all hsa-numbers('hsa:103') separated with a blank
	 * @return KG-Number, GeneSymbol, Gene Description, one per line. e.g.:
	 *         hsa:9693 RAPGEF2; Rap guanine nucleotide exchange factor (GEF) 2;
	 *         K08018 Rap guanine nucleotide exchange factor (GEF) 2 hsa:994
	 *         CDC25B; cell division cycle 25 homolog B (S. pombe)
	 *         (EC:3.1.3.48); K05866 cell division cycle 25B [EC:3.1.3.48]
	 */
	 ...
	 * 
	 * @param geneList
	 *            as a string array with kegg identifiers e.g. 'hsa:7529' list
	 *            size can be 1
	...
	public Definition[] getPathwaysWithTimeout(String org)
			throws TimeoutException {
	...
	public ArrayList<String> getPathwayListWithTimeout(String org)
			throws TimeoutException {
	...
	public SSDBRelation[] getBestBestNeighborsByGene(String genes_id,
		...
		SSDBRelation[] results = new SSDBRelation[0];
		try {
			results = serv.get_best_best_neighbors_by_gene(genes_id, offset,
					limit);
		} catch (RemoteException e) {
			e.printStackTrace();
		}
	...
	private static void printToScreen(SSDBRelation[] results) {
		...
		for (int i = 0; i < results.length; i++) {
			System.out.print("Start " + i + ": "
					+ results[i].getStart_position1() + " -> "
					+ results[i].getStart_position2());
			System.out.print(" \t|End " + i + ": "
					+ results[i].getEnd_position1() + " -> "
			...
			System.out.println(" \t|Overlap+Identity " + i + ": "
					+ results[i].getOverlap() + " --- "
					+ results[i].getIdentity());
		}
		System.out.println("XXXXXXXXXXXXXXXXXXXXXXXXXX");
	}
	 ...
	 * ORTHOLOGY KO: K04406 mitogen-activated protein kinase kinase kinase
	 * kinase 3 [EC:2.7.11.1] => Hier waere startWith "NAME" oder "DEFINITION".
	 * (Case insensitive)
	 * 
	...
	public static String extractInfo(String completeString, String startsWith,
		...
		int pos = completeString.toLowerCase().indexOf(
				"\n" + startsWith.toLowerCase()) + 1; // Prefer hits starting in
														// a new
		// line. // +1 because of \n
		if (pos < 0) {
			pos = completeString.toLowerCase()
					.indexOf(startsWith.toLowerCase());
		...
		if (endsWith == null || endsWith.length() != 0) {
			...
			while (Character.isWhitespace(completeString.charAt(sPos))
					&& completeString.charAt(sPos) != '\n')
				sPos++;
