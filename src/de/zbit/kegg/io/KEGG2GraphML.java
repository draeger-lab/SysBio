...
public class KEGG2GraphML implements KeggConverter {
	public static boolean silent = true; // Surpresses all outputs, except
											// %-values
	public static boolean absoluteNoOutputs = true;

	public static boolean retrieveKeggAnnots = true; // z.B. "hsa" = homo
														// sapiens, "" =>
														// General, null =>
														// keine infos von kegg.
	...
	public static boolean removeDegreeZeroNodes = true; // Bezieht sich nur auf
														// "Non-map nodes" (Also
														// nicht auf Pathway
														// referenzen).
	public static boolean renameCompoundToSmallMolecule = true; // Fuer Jochen &
																// GePS
																// unbedingt
																// true lassen
																// (so was wie
	...
	public static boolean lastFileWasOverwritten = false; // Gibt an, ob das
															// letzte
															// geschriebene
															// outFile bereits
															// vorhanden war und
	...
	public static void main(String[] args) {
		...
		if (args != null && args.length > 0) {
			...
			if (f.isDirectory())
				...
				KeggInfoManagement.saveToFilesystem("keggdb.dat", manager); // Remember
																			// already
																			// queried
																			// objects
			}
		...
		Pathway p = KeggParser.parse(
				"resources/de/zbit/kegg/samplefiles/_ko00010.xml").get(0); // 04115
		// Pathway p = KeggParser.parse("ko02010.xml").get(0);
		// p =
		// KeggParser.parse("http://kaas.genome.jp/kegg/KGML/KGML_v0.6.1/ko/ko00010.xml").get(0);
	...
	public void Convert(Pathway p, String outFile) {
		...
		if (retrieveKeggAnnots) {
			...
			/*
			 * for (Reaction r:p.getReactions()) { for (String
			 * ko_id:r.getName().split(" ")) { preFetchIDs.add(ko_id); } }
			 */
		...
		EdgeMap interactionDescription = graph.createEdgeMap(); // =
																// Subtype.name
																// "inhibition",
																// "activation",
																// "interaction",
																...
																// "control"
		// EdgeMap interactionType = graph.createEdgeMap(); // "control",
		// "catalysis", "physical interaction", "transport", "complex assembly",
		...
		int aufrufeGesamt = p.getEntries().size(); // +p.getRelations().size();
													...
													// schnell.
		if (!retrieveKeggAnnots)
			aufrufeGesamt += p.getRelations().size();
		...
		for (int i = 0; i < p.getEntries().size(); i++) {
			if (silent && !absoluteNoOutputs)
				progress.DisplayBar(null);
			if (!silent)
				System.out.println(i + "/" + p.getEntries().size());
			Entry e = p.getEntries().get(i);
			if (skipCompounds
					&& e.getType().toString().equalsIgnoreCase("compound"))
				continue;

			Node n;
			boolean isPathwayReference = false; // TODO: Anders Layouten ???
												// weglassen ???
			String name = e.getName().trim();
			if (name.toLowerCase().startsWith("path:")
					|| e.getType().equals(EntryType.map))
				isPathwayReference = true;

			if (e.hasGraphics()) {
				/*
				 * XXX: <entry id="16" name="ko:K04467 ko:K07209 ko:K07210"
				...
				if (g.getName().length() != 0)
					name = g.getName(); // + " (" + name + ")"; // Append ko
										// Id(s) possible!
				/*
				 * if (g.getWidth()>0 && g.getHeight()>0) { n =
				 * graph.createNode(g.getX(), g.getY(), g.getWidth(),
				 * g.getHeight(), name); } else { n = graph.createNode(g.getX(),
				 * g.getY(), name); }
				 */
				...
				if (e.getType().toString().trim().equalsIgnoreCase("group")) {
					...
					// New Text
					String newText = null;
					if (nl.getText().length() == 0
							|| nl.getText().equals("undefined")) {
						/*
						 * String newText = "Group of: " + "\n"; char sep = ',';
						 * for (Integer i2: e.getComponents()) { Entry e2 =
						 * p.getEntryForId(i2); if (e2==null) { newText+=i2+sep;
						 * } else if (e2.hasGraphics() &&
						 * !e2.getGraphics().getName().isEmpty()) {
						 * newText+=e2.getGraphics().getName()+sep; } else {
					...
					nl.setFontSize(10); // ein bischen kleiner, sieht bei
										// circles besser aus.
				} else if (g.getType().toString().equals("roundrectangle")) {
					nr = new ShapeNodeRealizer(ShapeNodeRealizer.ROUND_RECT);
					// } else if (g.getType().toString().equals("line")) { //
					// Vielleicht in neuerer yFiles version line verfuegbar?
					// nr = new ShapeNodeRealizer(ShapeNodeRealizer.TRAPEZOID);
				} else
					nr = new GenericNodeRealizer();

				try {
					if (g.getBgcolor() != null && g.getBgcolor().length() != 0
							&& !g.getBgcolor().trim().equalsIgnoreCase("none"))
				...
				try {
					if (g.getFgcolor() != null && g.getFgcolor().length() != 0
							&& !g.getFgcolor().trim().equalsIgnoreCase("none"))
				...
				if (isPathwayReference
						&& name.toUpperCase().startsWith("TITLE:")) {
				...
				// Links im Knotenlabel (und spaeter nochmal im Knoten)
				// speichern.
				String link = e.getLink();
				if (link != null && link.length() != 0) {
					try {
						nl.setUserData(new URL(link)); // In URL konvertieren,
														// da er auch type
														// speichert und URL
														// besser ist als STRING
					} catch (MalformedURLException e1) {
						nl.setUserData(link);
					}
				}
			...
			if (n != null) {
				...
				if (retrieveKeggAnnots) {
					...
					if (e.getName().trim().contains(" "))
						hasMultipleIDs = true;

					String name2 = "", definition = "", entrezIds2 = "", uniprotIds2 = "", eType = "", ensemblIds2 = "";
					for (String ko_id : e.getName().split(" ")) {
						// Definition[] results =
						// adap.getGenesForKO(e.getName(),
						// retrieveKeggAnnotsForOrganism); // => GET only (und
						// alles aus GET rausparsen). Zusaetzlich: in
						// sortedArrayList merken.
						if (ko_id.trim().equalsIgnoreCase("undefined"))
							continue;

						KeggInfos infos = new KeggInfos(ko_id, manager);

						// "NCBI-GeneID:","UniProt:", "Ensembl:", ... aus dem
						// GET rausparsen
						if (infos.queryWasSuccessfull()) {
							String oldText = graph.getRealizer(n)
									.getLabelText();

							String exName = infos.getNames();
							if (exName != null && exName.length() != 0) {
								int pos = exName.lastIndexOf(";");
								if (pos > 0 && pos < (exName.length() - 1))
									exName = exName.substring(pos + 1,
											exName.length()).replace("\n", "")
											.trim();

								if (!hasMultipleIDs) // Knotennamen nur
														// anpassen, falls nicht
														// mehrere IDs.
									graph.getRealizer(n).setLabelText(exName);
								else if (oldText.length() == 0) // ... oder wenn
																// er bisher
																// leer ist.
									graph.getRealizer(n).setLabelText(exName);
							}
							...
							if (e.getType().equals(EntryType.map)) { // => Link
																		// zu
																		// anderem
																		// Pathway
																		// oder
							...
							text = infos.getEntrez_id(); // KeggAdaptor.extractInfo(infos,
															// "NCBI-GeneID:",
															// "\n");
															// //adap.getEntrezIDs(ko_id);
							if (text != null && text.length() != 0)
								entrezIds2 += (entrezIds2.length() != 0 ? ","
										: "")
										+ text; // .replace(",", "");
							text = infos.getUniprot_id(); // KeggAdaptor.extractInfo(infos,
															// "UniProt:",
															// "\n");
															// //adap.getUniprotIDs(ko_id);
							if (text != null && text.length() != 0)
								uniprotIds2 += (uniprotIds2.length() != 0 ? ","
										: "")
										+ text; // .replace(",", "");
							text = infos.getEnsembl_id(); // KeggAdaptor.extractInfo(infos,
															// "Ensembl:",
															// "\n");
															// //adap.getEnsemblIDs(ko_id);
							if (text != null && text.length() != 0)
								ensemblIds2 += (ensemblIds2.length() != 0 ? ","
										: "")
										+ text; // .replace(",", "");

							// //eType+=(!eType.length()==0?",":"")+e.getType().toString();
							// eType+=(!eType.length()==0?",":"");
							// if (renameCompoundToSmallMolecule &&
							// e.getType().equals(EntryType.compound))
							// eType+="small molecule"; else
							...
							// Jochen: "protein", "protein in complex",
							// "complex", "RNA", "DNA", "small molecule",
							// "RNA in complex", "DNA in complex",
							// "small molecule in complex", "pathway",
							// "biological process"
							// Mapping: gene/ortholog => protein.
							// Group=>complex, compound=>complex,
							// map=>pathway(/biolog. process)
							// enzyme & other fehlen.
				...
				if (e.getLink() != null && e.getLink().length() != 0)
					nodeURLs.set(n, e.getLink());

				if (isPathwayReference)
					PWReferenceNodeTexts.add(graph.getRealizer(n)
							.getLabelText());
			}
		...
		for (int i = 0; i < parentGroupNodes.size(); i++) {
			...
			int offset = 5;
			graph.setLocation(parentGroupNodes.get(i), x - offset, y - offset
					- 14);
			graph.setSize(parentGroupNodes.get(i), width - x + 2 * offset,
					height - y + 2 * offset + 11);

			// Set hirarchie
			hm.setParentNode(nl, parentGroupNodes.get(i));

			// Reposition group node to fit content (2nd time is neccessary.
			// Maybe yFiles bug...)
			graph.setLocation(parentGroupNodes.get(i), x - offset, y - offset
					- 14);
			graph.setSize(parentGroupNodes.get(i), width - x + 2 * offset,
		...
		for (int i = 0; i < p.getRelations().size(); i++) {
			if (silent && !absoluteNoOutputs && !retrieveKeggAnnots)
				progress.DisplayBar(null);
			if (!silent)
				System.out.println(i + "/" + p.getRelations().size());
			...
			Edge myEdge;
			// XXX: Hier noch moeglich die Type der reaktion (PPI, etc.)
			// hinzuzufuegen.
			if (r.getSubtypes() != null && r.getSubtypes().size() > 0) {
				for (int stI = 0; stI < r.getSubtypes().size(); stI++) {
					...
					if (st.getName().trim().equalsIgnoreCase("compound")
							&& isNumber(st.getValue())) {
						Entry compNode = p.getEntryForId(Integer.parseInt(st
								.getValue()));
						if (compNode == null || compNode.getCustom() == null) {
						...
						if (value != null) {
							if (value.equals("-->")) {
								...
								er.setTargetArrow((Arrow.T_SHAPE)); // T_SHAPE
																	// erst ab
																	// yFiles
																	// 2.7
							} else if (value.equals("..>")) {
							...
							} else if (value.equals("-+-")) { // Ab 2.7 YFiles
																// Version
																// only...
							...
							if (st != null)
								interactionDescription
										.set(myEdge, st.getName()); // =>
																	...
																	// ubiquitination,
																	// methylation
							// Jochen: "inhibition", "activation",
							// "interaction", "physical interaction",
							// "catalysis", "transcriptional control", "control"
						}
					}
				}
			} else {
		...
		// Kanten von Knoten, welche exakt selben In- und Output haben
		// zusammenfassen. (=> Groupnode)
		if (groupNodesWithSameEdges) {
			...
			for (int i = 0; i < myNodes.length - 1; i++) {
				...
				if (hm.isGroupNode(myNodes[i])
						|| hm.getParentNode(myNodes[i]) != null
						|| !hm.isNormalNode(myNodes[i])
						|| myNodes[i].edges().size() < 1)
					continue;

				for (int j = i + 1; j < myNodes.length; j++) {
					if (hm.isGroupNode(myNodes[j])
							|| hm.getParentNode(myNodes[j]) != null
							|| !hm.isNormalNode(myNodes[j]))
						continue;

					// Wenn in selber (optischer) "Reihe" und selbe kanten, dann
					// groupen.
					if (graph.getRealizer(myNodes[i]).getX() == graph
							.getRealizer(myNodes[j]).getX()
							|| graph.getRealizer(myNodes[i]).getY() == graph
									.getRealizer(myNodes[j]).getY())
				...
				if (nl.size() > 1) {
					...
					GroupNodeRealizer gnr = (GroupNodeRealizer) setupGroupNode(
							new NodeLabel(), "");
		...
		if (removeDegreeZeroNodes) { // Verwaiste knoten loeschen
			...
			for (Node n : nodes) {
				if (PWReferenceNodeTexts.contains(graph.getRealizer(n)
						.getLabelText()))
					continue; // Keine Links auf andere Pathways removen.
		...
		/*
		 * for (int i=0; i<parentGroupNodes.size(); i++) { NodeList nl = new
		 * NodeList(); double
		 * x=Double.MAX_VALUE,y=Double.MAX_VALUE,width=0,height=0; for (int n2:
		 * groupNodeChildren.get(i)) { Entry two = p.getEntryForId(n2); if
		 * (two==null) { System.out.println("WARNING: Missing node for id " +
		 * n2); continue; } Node twoNode = (Node) two.getCustom(); NodeRealizer
		 * nr = graph.getRealizer(twoNode); x = Math.min(x, nr.getX()); y =
		 * Math.min(y, nr.getY()); width=Math.max(width,
		 * (nr.getWidth()+nr.getX())); height=Math.max(height,
		 * (nr.getHeight()+nr.getY()));
		 * 
		 * nl.add(twoNode); }
		 * 
		 * // Reposition group node to fit content int offset = 5;
		 * graph.setLocation(parentGroupNodes.get(i), x-offset, y-offset-14);
		 * graph.setSize(parentGroupNodes.get(i), width-x+2*offset,
		 * height-y+2*offset+11);
		 * 
		 * // Set hirarchie hm.setParentNode(nl, parentGroupNodes.get(i));
		 * 
		 * // Reposition group node to fit content (2nd time is neccessary.
		 * Maybe yFiles bug...) graph.setLocation(parentGroupNodes.get(i),
		 * x-offset, y-offset-14); graph.setSize(parentGroupNodes.get(i),
		 * width-x+2*offset, height-y+2*offset+11);
		 * 
		 * }
		 */
		...
		if (outputHandler != null && outputHandler.canWrite()) {
			...
			if (outputHandler instanceof GraphMLIOHandler) {
				Graph2DGraphMLHandler ioh = ((GraphMLIOHandler) outputHandler)
						.getGraphMLHandler();
				...
				addEdgeMap(interactionDescription, ioh,
						"interactionDescription");
			}
			...
			while (retried < 3) {
				try {
					...
					retried++;
					if (retried > 2)
						System.err.println("Error while encoding file "
								+ outFile + "\n" + iex);
				}
			}
		} else {
	...
	private static void addNodeMap(NodeMap nm, Graph2DGraphMLHandler ioh,
			String desc, KeyType keytype) {
	...
	private static void addEdgeMap(EdgeMap nm, Graph2DGraphMLHandler ioh,
			String desc, KeyType keytype) {
	...
	private static NodeList removeOutlier(NodeList nl, Graph2D graph) {
		...
		for (int j = 0; j < nl.size(); j++) {
			...
			for (int k = 0; k < nl.size(); k++) {
				...
				double dist = Math.max(Math.abs(n1.getCenterX()
						- n2.getCenterX()), Math.abs(n1.getCenterY()
						- n2.getCenterY()));
	 ...
	 * @param nl
	 * @param changeCaption
	 *            = null if you don't want to change the caption.
	 */
	private static NodeRealizer setupGroupNode(NodeLabel nl,
		...
		nr.setMinimalInsets(new YInsets(5, 2, 2, 2)); // top, left, bottom,
														// right
		nr.setAutoBoundsEnabled(true);
		nl.setPosition(NodeLabel.TOP);
		nl.setBackgroundColor(new Color((float) 0.8, (float) 0.8, (float) 0.8,
				(float) 0.5));
		nl.setFontSize(10);
	...
	/**
	 * Tests, if two nodes do have exactly the same source and target nodes on
	 * their edges and if the edges do have the same shape (Arrow, LineType and
	 * description).
	 * 
	 * @param n1
	 * @param n2
	...
	public static boolean nodesHaveSameEdges(Node n1, Node n2, Graph2D graph) {
		if (n1.inDegree() != n2.inDegree() || n1.outDegree() != n2.outDegree())
			return false;
		...
		while (e != null) {
			...
			if (!found)
				return false; // Edge from n1 not in n2
			e = e.nextInEdge();
		}
	...
	public static boolean edgesEqualExceptTargets(Edge e, Edge e2, Graph2D graph) {
		...
		if (er.equals(er2) || er.getTargetArrow().equals(er2.getTargetArrow())
				&& er.getSourceArrow().equals(er2.getSourceArrow())
				&& er.getLineType().equals(er2.getLineType())
				&& er.getLabelText().equals(er2.getLabelText())) {
			return true;
		}
		return false;
	}
	...
	public static Color ColorFromHTML(String theColor) {
		...
		if (theColor.trim().equalsIgnoreCase("none"))
			theColor = "000000";

		if (theColor.length() != 6)
			throw new IllegalArgumentException("Not a valid HTML color: "
					+ theColor);
		return new Color(Integer.valueOf(theColor.substring(0, 2), 16)
				.intValue(), Integer.valueOf(theColor.substring(2, 4), 16)
				.intValue(), Integer.valueOf(theColor.substring(4, 6), 16)
				.intValue());
	}
	...
	private static void configureView(Graph2DView view) {
		...
		// view.setSize(dim);
		view.zoomToArea(box.getX() - 10, box.getY() - 10, box.getWidth() + 20,
				box.getHeight() + 20);
		view.fitContent();
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.zbit.kegg.io.KeggConverter#Convert(java.lang.String,
	 * java.lang.String)
	 */
