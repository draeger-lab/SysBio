...
public class KEGG2jSBML implements KeggConverter {
	public static boolean retrieveKeggAnnots = true; // Retrieve annotations
														// from Kegg or use
														// purely information
														// available in the
														// document.
	...
	public static void main(String[] args) {
		...
		if (args != null && args.length > 0) {
			...
			KeggInfoManagement.saveToFilesystem("keggdb.dat", k2s
					.getKeggInfoManager()); // Remember already queried objects
		...
		try {
			k2s.Convert("resources/de/zbit/kegg/samplefiles/hsa00010.xml",
					"resources/de/zbit/kegg/samplefiles/hsa00010.sbml.xml");
	...
	public void Convert(Pathway p, String outfile) {
		...
		/*
		 * Bisher: - Comportment mit / enden - <listOfCompartments> -
		 * <listOfSpecies> - <none> entfernen
		 */
	...
	public SBMLDocument Kegg2jSBML(Pathway p) {
		...
		Compartment compartment = model.createCompartment(); // Create
																// neccessary
																// default
																// compartment
		...
		for (Entry entry : entries) {
			...
			specAnnot.setAbout("");
			spec.setAnnotation(specAnnot); // manchmal ist jSBML schon
											// bescheurt...
			...
			if (treatEntrysWithReactionDifferent && entry.getReaction() != null
				...
				spec.setSBOTerm(ET_SpecialReactionCase2SBO); // TODO: Ist es
																// richtig,
																// sowohl dem
																// Modifier als
																// auch der
				...
				ModifierSpeciesReference modifier = new ModifierSpeciesReference(
						spec);
				modifier.setSBOTerm(461); // Entymatic catalyst modifier //
											// TODO: ist wirklich jeder Entry
											// mit Reaction eine Enzymatic-Unit?
			...
			if (name.toLowerCase().startsWith("path:")
					|| entry.getType().equals(EntryType.map))
			...
			for (String ko_id : entry.getName().split(" ")) {
				...
				cvtKGID.addResource(KeggInfos.getMiriamURIforKeggID(ko_id,
						entry.getType()));
				...
				if (infos.queryWasSuccessfull()) {
					...
					spec
							.appendNotes(String
									.format(
											"<p><b>Description for &#8220;%s&#8221;:</b> %s</p>\n",
											EscapeChars
													.forHTML(infos.getName()),
											EscapeChars.forHTML(infos
													.getDefinition())));
					if (infos.containsMultipleNames())
						spec.appendNotes(String.format(
								"<p><b>All given names:</b> %s</p>\n",
					...
					sbReaction
							.appendNotes(String
									.format(
											...
											ko_id.toUpperCase())); // Experimental...
		...
		if (addCellDesignerAnnots)
			addCellDesignerAnnotationSuffixToModel(annot);
	...
	private void configureReactionComponent(Pathway p, ReactionComponent rc,
		...
		sr
				.setSpecies((spec != null && spec.getCustom() != null) ? (Species) spec
						.getCustom()
						: null);
	...
	private void addCellDesignerAnnotationPrefixToModel(Pathway p,
			Annotation annot) {
		annot
		...
		annot.appendNoRDFAnnotation("<celldesigner:modelDisplay sizeX=\""
				+ (maxCoords[0] + 2) + "\" sizeY=\"" + (maxCoords[1] + 2)
				+ "\"/>\n");
		annot
	...
	private void addCellDesignerAnnotationSuffixToModel(Annotation annot) {
		...
		/*
		 * TODO: </celldesigner:listOfSpeciesAliases>
		 * <celldesigner:listOfGroups/> <celldesigner:listOfProteins>
		 * <celldesigner:protein id="pr1" name="s1" type="GENERIC"/>
		 * </celldesigner:listOfProteins> <celldesigner:listOfGenes/>
		 * <celldesigner:listOfRNAs/> <celldesigner:listOfAntisenseRNAs/>
		 * <celldesigner:listOfLayers/> <celldesigner:listOfBlockDiagrams/>
		 */
	}
