...
public abstract class Launcher implements Runnable, Serializable {
  ...
  private Launcher(boolean showCopyrightMessage) {
    ...
    try {
       ...
       */
      de.zbit.gui.GUITools.configureSplashScreen(getVersionNumber(),
  ...
  public Launcher(Launcher launcher) {
    this(false);
    props = launcher.getCommandLineArgs().clone();
    terminateJVMwhenDone = launcher.isTerminateJVMwhenDone();
  }
  ...
  public void checkForUpdate() throws MalformedURLException {
    ...
    if (url != null) {
      de.zbit.gui.UpdateMessage update = new de.zbit.gui.UpdateMessage(
        false, getAppName(), url, getVersionNumber(), true);
      update.execute();
    }
  }
  ...
  public void guiMode(AppConf appConf) {
    ...
    if (ui != null) {
      ...
      de.zbit.gui.GUITools.hideSplashScreen();
      ui.toFront();
    } else {
      if (appConf.getCmdArgs().getBooleanProperty(de.zbit.gui.GUIOptions.GUI)) {
  ...
  protected void launchCommandLineMode(AppConf appConf) {
    if (appConf.getCmdArgs().getBoolean(de.zbit.gui.GUIOptions.CHECK_FOR_UPDATES)) {
      try {
        checkForUpdate();
      ...
      javax.swing.SwingUtilities.invokeLater(new Runnable() {
        ...
        public void run() {
          try {
            ...
            de.zbit.gui.GUITools.initLaF();
            guiMode(appConf);
          } catch (java.awt.HeadlessException exc) {
            if (props.getBooleanProperty(de.zbit.gui.GUIOptions.GUI)) {
  ...
  public boolean showsGUI() {
    return (props.size() < 1) || props.getBooleanProperty(de.zbit.gui.GUIOptions.GUI);
  }
