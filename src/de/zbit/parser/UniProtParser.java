...
public class UniProtParser {
  ...
  /**
   * @param all uniprot identifiers (must be uniprot id!) 
   */
  public List<String[]> getGene(String[] identifier) {
    ...
    for (int i=0; i<proteins.length; i++) {
      ...
      if (p==null) {
        //        System.out.println("identifier " + identifier[i] + " gibt's nicht.");
        continue;
      }
      ...
      for (String string : splitLine) {
        ...
        if (splitEntry[0].equals("ID")) {
          protein = splitEntry[1];
        }
        else if (splitEntry[0].equals("GN") && !string.equals("GN\tAND") && (splitEntry.length>1)) {
          int pos = splitEntry[1].indexOf(';');
          if (pos > 0)
            splitEntry[1] = splitEntry[1].substring(0,pos - 1);
          split = splitEntry[1].split("=");
          if (split[0].equals("NAME") && split.length >1) { //TODO: Vorsicht hier wurden komplett die Synonyme ignoriert!
            gene = split[1];
            genes.add(new String[] { protein, gene });
          }
          //          else{
          //            System.out.println("hier wäre wieder die Fehlermeldung: " + protein + " - "+ string);
          //          }
        }
      }
    }
