  ...
  public List<String[]> getGene(String[] identifier) {
    ...
    for (int i=0; i<proteinBlock.length; i++) {
      ...
      for (String line : splitLines) {
        ...
        String[] splitLineEntry = line.split("\t");
        if (splitLineEntry[0].equals("GN") && !line.equals("GN\tAND") && (splitLineEntry.length>1)) {
//          GN   Name=<name>; Synonyms=<name1>[, <name2>...]; OrderedLocusNames=<name1>[, <name2>...];
//          GN   ORFNames=<name1>[, <name2>...];
//          GN   and
          String[] geneSplit = splitLineEntry[1].split(";");
          for (String gs : geneSplit) {
            gs = gs.trim();
            String[] split = gs.split("=");
            if(split.length==2){          //TODO: Vorsicht hier wurden komplett die Synonyme ignoriert! Nur Name geparst
              if(split[0].trim().equals("NAME")){
                genes.add(new String[]{identifier[i], split[1].trim()});
              }
            }
          }
...
//      if(i==0 || i==100 || i==40000){
//        System.out.println("============BLOCK=================");
//        System.out.println(block);
////        System.out.println("-------------" + protein + " " + gene);
...
//      }
  ...
  public static void main(String[] args) {
        ...
        List<String[]> test = uw
                  .getGene(new String[]{"A1A4B1_MOUSE"});//, "1433B_MOUSE"});
