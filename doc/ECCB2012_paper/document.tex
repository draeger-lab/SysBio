...
But until now, there exists mainly converters from SBML to BioPax like \emph{The System Biology Format Converter} (\citealp[see][]{Group2011}) but no converter for BioPax to SBML, whose conversion would also include relations.
...
Depending of the kind of the \texttt{PhysicalEntity}, i.e. if it is a protein, complex, DNA, RNA, or small molecule, the species is annotated with the corresponding SBO term \textbf{TODO Clemens: SBO citation}.
...
Finally, an RDF link from the \texttt{PhysicalEntity} to a corresponding Entrez Gene ID is searched, because the Gene ID facilitates the annotation in the fifth step.
...
\texttt{Conversion}s are mainly translated into SBML reactions except of the \texttt{Transport} and the \texttt{Degradation} elements.
...
%%%%%%%%%%% Beispiel %%%%%%%%%%%%%%%%%%%%
\textbf{TODO Clemens: Rewrite this example!!!}
...
All supported identifiers from the BioPax files are parsed, some are manually curated (see example of inconsistent usage above) and some annotations are supplemented by additional queries to the KEGG API for every translated element.
